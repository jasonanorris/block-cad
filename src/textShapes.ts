...
import { isTextFont, type TextFont } from './textFonts'
...
const fonts = { helvetiker: fontData, 'helvetiker-bold': boldData, optimer: optimerData }
const parsedFonts = new Map<TextFont, ReturnType<FontLoader['parse']>>()
...
export function textContours(text: string, fontSize: number, fontId: TextFont = 'helvetiker'): { contours: SvgContours[]; width: number; depth: number } {
  if (!isTextFont(fontId)) throw new Error('Choose a supported text font.')
  const data = fonts[fontId]
  let font = parsedFonts.get(fontId)
  if (!font) { font = new FontLoader().parse(data); parsedFonts.set(fontId, font) }
...
export function createTextObject(text: string, fontSize: number, height: number, workplaneHeight = 0, fontId: TextFont = 'helvetiker'): Extract<CadObject, { type: 'text' }> {
  if (!Number.isFinite(height) || height <= 0 || height > 200) throw new Error('Text extrusion must be greater than 0 and at most 200 mm.')
  const result = textContours(text, fontSize, fontId)
  return { id: crypto.randomUUID(), name: text.replace(/\s+/g, ' ').trim().slice(0, 80), type: 'text', text, fontSize, fontId,
...
export function changeText(object: Extract<CadObject, { type: 'text' }>, text: string, fontSize: number, height: number, fontId: TextFont = object.fontId ?? 'helvetiker'): CadObject {
  const next = createTextObject(text, fontSize, height, 0, fontId)
  return { ...object, text, fontSize, fontId, contours: next.contours, dimensions: next.dimensions }
}
