...
To join shapes, Shift+click at least two separate solids and choose **Join**. You can also select holes linked to those solids at the same time. The workspace and STL export use one Manifold union of the solids, with any holes linked to a member cut from the joined result. Select a joined member in the Objects list to edit its original shape; members other than the first appear as a wireframe while selected. Moving any joined member shifts all the joined solids and their grouped holes together. Rotating and scaling a joined member still edits only that member and its grouped holes. Choose **Separate** with a joined member selected to restore separate solids. Joining, separating, and edits are undoable. Duplicating multiple joined members creates a new join among the copies; duplicating only one member makes an independent solid.
...
`src/groupTransforms.ts` applies group transforms to CAD data. Position and rotation preserve each grouped hole's offset and orientation. Scale applies the solid's relative transform to each hole. The object model has position, rotation, and axis scale but no shear, so a nonuniform scale of a rotated hole is approximated when decomposed back into those fields.
