...
`src/projectFile.ts` writes version 7 project JSON with `format: "block-cad"`, `units: "mm"`, and an `objects` array. A hole shape stores the ID of the solid it cuts. Joined solids share a join group ID. Optional object names are saved with the shapes. Versions 1 through 6 still load. Loading validates the file, shape data, names, hole links, and join groups before replacing the scene. Project files contain CAD data only; selection, undo history, generated meshes, and Three.js state are not saved.
