...
To cut a hole, add a cylinder and another solid shape, select the cylinder, and choose **Hole** under Shape mode. Use **Cut solid** to choose a box, solid cylinder, or sphere as the target. Move or resize the hole cylinder so it overlaps the target. Choose **Solid** to restore it as a separate shape. **Add cutout example** provides a ready-made box and cylinder hole. Deleting the hole restores its target; deleting the target turns its hole cylinder into a normal cylinder. Turning a solid cylinder into a hole also restores any cylinders that were cutting it to solids. Undo and Redo cover these changes.
...
`src/projectFile.ts` writes version 3 project JSON with `format: "block-cad"`, `units: "mm"`, and an `objects` array. A hole cylinder stores the ID of the solid it cuts. Version 1 and 2 files still load. Loading validates the file, shape data, and hole links before replacing the scene. Project files contain CAD data only; selection, undo history, generated meshes, and Three.js state are not saved.

`src/stlExport.ts` creates a binary STL from the same primitive sizes and transforms used in the workspace. It exports a cut target as the Manifold result and omits the hole cylinder as a separate solid. STL has no unit metadata, so import the file as millimeters in a slicer. Other overlapping shapes are still separate surfaces; use a slicer's union/merge option if needed for a single solid. Export STL is disabled for an empty project.

`src/booleanGeometry.ts` uses the Manifold WebAssembly library to derive the cut surface from the original CAD target and cylinder. Both the preview and STL export call this module. `src/useBooleanPreview.ts` manages generated preview geometry and disposes it when it changes. The Manifold result is derived data; edits, undo/redo, and project files retain the source shapes.
