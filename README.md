...
A simple browser-based 3D modeling project. You can add, select, move, rotate, size, duplicate, delete, undo, and redo changes to boxes, cylinders, and spheres on a gridded 3D workplane. Projects can be saved to and loaded from JSON files.
...
Open the local URL printed by Vite. Use New to start with an empty workplane, Save to download a project JSON file, and Load to open one. New and Load clear selection and undo history. An invalid file leaves the current project intact and displays an error. Use the Shapes buttons to add a box, cylinder, or sphere. New shapes appear on the workplane and are selected automatically. Choose Move, Rotate, or Scale above the workspace and drag the colored handles on the selected shape. The selected object's inspector also lets you enter position, rotation, and dimensions numerically. Use Duplicate or Delete in the sidebar, Ctrl/Cmd+D to duplicate, Delete or Backspace to remove, and Escape to clear selection. Undo and Redo are above the workspace; Ctrl/Cmd+Z undoes, Ctrl/Cmd+Shift+Z or Ctrl+Y redoes. App shortcuts do not run while editing an inspector field. Drag outside the handles to orbit, scroll to zoom, and right-drag to pan.
