...
[Quick user guide](docs/USER_GUIDE.md) · [Release checks and known limits](docs/RELEASE_CHECKS.md)
...
Enable **Section view**, choose a world axis and a position through the selected bodies, then **Split selected at section**. Each finished solid or joined assembly becomes two capped mesh solids at the original location. The section preview turns off, and both pieces are selected. Source color is retained; joins and linked holes are baked into the surfaces. Undo restores all original editable sources and the selection in one step. Both sides must contain material for every selected body, and all moving dependencies must be visible/unlocked. Invalid, empty, tangent, or outside cuts reject the entire operation. Results use the existing mesh format and 100,000-triangle limit per half; they can be transformed, cut, joined, saved, and exported. Flip side affects only the section preview.

### 70 — Text font choices

Text creation and editing now offer **Helvetiker Regular**, **Helvetiker Bold**, and **Optimer Regular**. The fonts are bundled; no network font service is needed. Changing the font rebuilds the lettering while retaining ID, placement, color, and hole links, and supports Undo/Redo. Unsupported characters are rejected. Project format 16 stores the chosen font ID alongside the sampled outlines and remains compatible with versions 1–15. Old text defaults to Helvetiker Regular when edited. The bundled font attribution is in `public/licenses/bundled-fonts.txt`. Custom font uploads are not included.
...
**Library backup / transfer** downloads all local parts and snapshots as a versioned JSON backup. Import validates all embedded projects before writing, preserves names/dates/model metadata, and adds fresh saved-item IDs without overwriting existing entries or changing the open model. Reimporting creates duplicates. Parts and snapshots remain separate; empty snapshots are supported. Writes use one transaction across metadata and model stores, so quota/storage failures leave no partial imports. The saved lists refresh after import. Limits are 50 MB, 250 entries, and 10,000 source shapes per backup. Autosave, unsaved current work, camera/workplane preferences, and Undo history are excluded; use project Save for the open model. Library transfer is separate from modeling Undo. Browser regression checks are available in `tests/browser-library-transfer.mjs`.

### 73 — Reusable custom shapes

**Shapes → Custom shapes** adds parameterized tubes, rounded boxes, and upright L brackets. Select one and use **Edit custom shape** to revise dimensions, wall thickness, or radius while retaining its transform, color, ID, and links. Parameter dimensions describe the unscaled source; inspector sizing remains a separate scale. Tubes have concentric openings; rounded boxes round vertical corners with flat tops/bottoms; L brackets have a base and upright leg. Parameters persist through duplication, arrays, projects, parts, snapshots, and library backups. Project format 17 validates parameters and matching dimensions and reads formats 1–16. Source previews and Manifold use the same sampled profiles, so cuts, joins, splits, and geometry exports work consistently. Dimension limits are 0.01–1,000 mm; radius may be zero, and walls/radius must fit the profile.

### 74 — Hole patterns

**Combine → Hole patterns** creates rows, grids, and bolt circles with diameter, depth, count, spacing, start angle, and plane offsets. Select a solid target; the tool includes its entire joined body. Without a face workplane, patterns are centered on its finished bounds and cut downward from its highest Y. With a face workplane, they start at the picked origin and cut inward along its normal. A 0.02 mm extension above the entry plane avoids coincident faces; the inward depth remains the requested value. Up to 200 ordinary grouped cylinder cutters are added in one Undo step and remain individually editable. Hidden/locked target members and invalid inputs are rejected. Cutters outside the target do not cut it. Choose enough depth for through holes; there is no automatic through-depth mode or persistent pattern constraint.

### 75 — Face alignment

Select one body and use **Place → Align faces** to pick its moving face and a separate body's target face. **Apply face alignment** rigidly rotates and translates the whole selected assembly, including every linked cutter, so the picked points coincide and outward normals oppose. Positive offsets leave a gap along the target normal; negative offsets overlap. The target stays fixed and may be locked. Moving hidden/locked dependencies are rejected; selection/model changes invalidate picks. The operation is one Undo step, preserves signed/nonuniform scales without adding shear, and uses the shortest normal rotation. Faces are sampled mesh triangles; other collisions and additional twist are not resolved.
